...
setup_buildx() {
    ...
    if ! docker buildx inspect pixospritz-builder >/dev/null 2>&1; then
        docker buildx create --name pixospritz-builder --driver docker-container --bootstrap
    fi

    # Use the builder
    docker buildx use pixospritz-builder
}
