...
This repository contains a Dockerfile and build script for compiling the Pixospritz game engine and necessary kernel modules for Anbernic's Linux-based handheld console devices based on the Allwinner h700 chip. Broadly, these are the RG35XX* & RG40XX* devices
...
The idea behind this is to provide a platform that makes it relatively easy for most people to compile a runnable executable of the Pixospritz game engine for handheld gaming consoles. With a bit more tinkering, this could be made more flexible so that it's easy to build Pixospritz for other devices and/or other firmware.
...
- A folder named `pixospritz` with the `pixospritz` executable and the compiled kernel modules (`cdc-acm.ko`, `snd-hwdep.ko`, `snd-usbmidi-lib.ko`, `snd-usb-audio.ko`) inside
- `pixospritz.sh` startup script
...
- Drop the `pixospritz` folder and the `pixospritz.sh` script into this location
- (Optional) Add the pixospritz application to the `gamelist.xml` file to make it a bit more pretty in the Knulli UI

## Troubleshooting

If you encounter issues running pixospritz on your device, here are some common problems and solutions:

### "Permission denied" Errors
When you see "Permission denied" or the app won't launch:

1. Make sure both files are executable by running these commands in the ports/pixospritz directory:
   ```bash
   chmod +x pixospritz.sh
   chmod +x pixospritz/pixospritz
   ```
...
1. Check existing [Issues](https://github.com/jamesMcMeex/pixospritz-rg35xx-knulli/issues) to see if others have solved your problem
...
## Acknowledgments
- [kderbyma](https://github.com/kderbyma) for developing Pixospritz game engine
- [jamesMcMeex](https://github.com/jamesMcMeex) for the original build system
